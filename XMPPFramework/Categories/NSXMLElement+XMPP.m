 ...
 *  @return IQ体
 */
+(IQElement *)iqElementWithFrom:(NSString *)fromJID to:(NSString *)toJID type:(NSString *)type id:(NSString *)ID childElement:(ChildElement *)childElement
{
    IQElement *iqElement = [NSXMLElement elementWithName:@"iq"];
    if (fromJID) {
        [iqElement addAttributeWithName:@"from" stringValue:fromJID];
    }
    if (toJID) {
        [iqElement addAttributeWithName:@"to" stringValue:toJID];
    }
    if (ID) {
        [iqElement addAttributeWithName:@"id" stringValue:ID];
    }
    ...
    [iqElement addChild:childElement];
    return iqElement;
}
/**
 *  创建子节点
 ...
 *  @param xmlns        节点xmlns属性
 *  @param dicAttribute 节点的attribute属性字典
 *  @param value        字符串
 *
 *  @return IQ的子节点
 */
+(ChildElement *)childElementWithName:(NSString *)elementName xmlns:(NSString *)xmlns attribute:(NSDictionary *)dicAttribute stringValue:(NSString *)value
{
    ChildElement *childElement = [ChildElement elementWithName:elementName xmlns:xmlns];
    
    [[dicAttribute allKeys] enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        id object = [dicAttribute objectForKey:obj];
        if ([object isKindOfClass:[NSNumber class]]) {
            [childElement addAttributeWithName:obj numberValue:object];
        }else if([object isKindOfClass:[NSString class]]){
            [childElement addAttributeWithName:obj stringValue:object];
        }else{
            [childElement addAttributeWithName:obj objectValue:object];
        }
    }];
